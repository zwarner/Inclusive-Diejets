...
TH1D *h1 = new TH1D("pt1", "Dijets", 30,0,150);
TH1D *h2 = new TH1D("pt2", "Dijets", 30,0,150);
TH1D *h3 = new TH1D("Detal Phi", "Dijets",20,0,3.2);
TH1D *h4 = new TH1D("Delta Eta", "Dijets",20,0,10);
TH1D *h5 = new TH1D("Multiplicity","Dijets",50,0,50);
TH1D *h6 = new TH1D("Number of Charged Particles","Dijets",30,0,200);
...
int main1()
{
  ...
  for(int i = 0; i<numEvents; i++) 
      ...
      for(int j = 0; j < t.kMaxParticle; j++)//loops over stable particles
	{
	 if(t.Particle_Status[j] == 1) // makes sure the particle is stable and creates a PseudoJet vector
	      ...
	      if(sqrt(temp_px*temp_px + temp_py*temp_py) > .0000001 && sqrt(temp_px*temp_px + temp_py*temp_py) < 13000 )//makes sure values are reasonable (real)
		 {
		     px = t.Particle_Px[j];
		     py = t.Particle_Py[j];
		     pz = t.Particle_Pz[j];
			  ...
			  for(int j = 0; j < t.kMaxParticle; j++)
			    {
				 ...
				 if(abs(t.Particle_Eta[j]) < 1.0 && t.Particle_PT[j] > 1.25)
  ...
  gStyle->SetOptTitle(kFALSE);
   ...
   cmslabel.DrawLatex(0.5,0.55,"PT_{cp} > 1.0 GeV");  
